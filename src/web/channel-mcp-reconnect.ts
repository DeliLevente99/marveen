 ...
 * menu in the agent's session. Generalises the existing
...
export function attemptChannelMcpReconnect(agentName: string): ReconnectResult {
  ...
  try {
    agentRuntime.sendKey(session, 'Escape')
    agentRuntime.sleepSync(1000)

    agentRuntime.sendText(session, '/mcp')
    ...
    for (let upCount = 1; upCount <= MAX_UP_ATTEMPTS; upCount++) {
      agentRuntime.sendKey(session, 'Up')
      agentRuntime.sleepSync(200)
      agentRuntime.sendKey(session, 'Enter')
      agentRuntime.sleepSync(1000)
    ...
    agentRuntime.sendKey(session, 'Down')
    ...
    try { agentRuntime.sendKey(session, 'Escape') } catch { /* best effort */ }
